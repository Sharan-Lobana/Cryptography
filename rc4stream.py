...
def encipher(plaintext,s,bit_stream = False):
	copy = s[:]
	if not bit_stream:
		...
		while(index != length):
			...
			s[i],s[j] = s[j],s[i] 
			k = (int(s[i],base=2)+int(s[j],base=2))%256
			cipher += '{0:08b}'.format(int(plaintext[8*index:8*(index+1)],base=2)^int(s[k],base=2))
			index += 1
		return cipher,copy


def decipher(cipher,s,bit_stream=False):
	length = len(cipher)/8
	i = 0
	j = 0
	plaintext = ''
	...
	while(index != length):
		...
		if not bit_stream:
			...
			plaintext += '{0:08b}'.format(int(cipher[8*index:8*(index+1)],base=2)^int(s[k],base=2))
		index += 1
	return plaintext